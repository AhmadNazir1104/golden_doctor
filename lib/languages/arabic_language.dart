 Map<String, String> arabic = {
  'formTitle': 'نموذج لغة متعددة',
  'enterText': 'أدخل النص هنا',
  'enterName': 'أدخل اسمك',
  'textOne': 'النص الأول',
  'textTwo': 'النص الثاني',
  'textOne3': '3 Text',
  'textTwo4': '4 Text',
  // Splash Screen
  'Master the Game ': 'إتقان اللعبة',
  'of Today’s Fashion': 'من موضة اليوم',
  'Scrubser  here. Anytime. Anywhere.': 'المغسلة هنا . في أي وقت. في أي مكان.',
  'Get Started': 'ابدأ',

  // Well Come Screen
  "Let's Shop Now!": "دعونا نتسوق الآن!",
  "Dive in and discover a world of possibilities\n waiting for you.":
      "انغمس واكتشف عالمًا من الاحتمالات في انتظارك.",
  "Log in": "تسجيل الدخول",
  "Create an Account": "إنشاء حساب",
  "or": "أو",
  "Continue as a Guest": "الاستمرار كضيف",

  // Login Screen
  "Welcome Back, Shopioholic!": "مرحبًا بك مرة أخرى، محبي التسوق!",
  "We are thrilled to have you back! Log in to Shopio and shop till you drop.":
      "نحن سعداء بعودتك! قم بتسجيل الدخول إلى شوبيو وتسوق حتى تسقط.",
  "Enter your email": "أدخل بريدك الإلكتروني",
  "Enter your password": "أدخل كلمة المرور الخاصة بك",
  "Forget Password?": "هل نسيت كلمة السر؟",
  "Sign In": "تسجيل الدخول",
  "Don`t have an account?": "ليس لديك حساب؟",
  "Sign Up": " اشتراك",
  "By continuing, I agree to ": "من خلال الاستمرار، أوافق على ",
  "Terms of Conditions ": "شروط الشروط ",
  "and\n": "و\n",
  "Privacy of Policy ": "خصوصية السياسة ",

  //  Signup Screen
  "Welcome!": "مرحباً!",
  "Confirm Password": "تأكيد كلمة المرور",
  "Already have an account?": "هل لديك حساب بالفعل؟",
  " Try Again ": "حاول ثانية ",

  // Forget Password Screen
  "Enter the email address associated with your account.":
      "أدخل عنوان البريد الإلكتروني المرتبط بحسابك.",
  "We will email you a link to restart your password.":
      "سنرسل إليك عبر البريد الإلكتروني رابطًا لإعادة تشغيل كلمة المرور الخاصة بك.",
  "Enter email": "أدخل البريد الإلكتروني",
  "Send": "يرسل",
  "Forget Password":"هل نسيت كلمة السر",
  

  // Home Screen
  "See All": "رؤية الكل",

  // Collection Screen
  "Filter": "تصفية حسب",
  "COLOR": "لون",
  "FILTER BY": "تصفية حسب",
  "CLEAR ALL": "مسح الكل",
  "VIEW": "منظر",
  "Gender": "جنس",


    // Quick By
  "Quick By": "Quick By",
  "ahmed":"dev"
};
